...
Confirming `analysis/united-states-1947/threshold-arithmetic.md`, Z3 printed a House quorum of 219 where the hand derivation gives 218. The solver was right and the label was wrong — the encoding minimized `yes` rather than `present`, making 219 an arbitrary witness among models achieving 146 yes votes. Minimizing `present` gives 218.

Nothing was unsound. The formula answered exactly what was written, and the sentence attached to the output claimed something the formula did not say. Every tool in the table above is exposed to this, and none of them checks it. It is recorded at greater length in `analysis/united-states-1947/formal-model-replication.md`, in the file whose main result is a successful machine-checked proof, because that is where the caution is most needed.
